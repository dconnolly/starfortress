...
\begin{proof}
    ...
    \item[\emph{Proof}]
        ...
        \begin{gamefigure}{Adversary $\adversary{B}$ against the IND-CCA security of \kem. Note that both place at most $q_d$ queries to $\dec_O$, one for each decapsulation query placed by $\adversary{A}$.}{fig:adversary-B}
            \begin{games}{2}
    ...
    \item[\emph{Proof}]
        We construct $\adversary{E}$ as in Figure \ref{fig:adversary-E}, and the analysis is similar to the previous hop.
    ...
    \item[\textbf{Game 4}]
        ...
        Finally, we revert the changes introduced in Game 1 and use a real key for $k_1^*$ once more.
    ...
    \item[\emph{Claim 4}]
        ...
        We introduce adversary $\adversary{C}$ against the IND-CCA security of \kem, with roughly the same running time as $\adversary{A}$, in Figure \ref{fig:adversary-C}.
    ...
    \item[\emph{Proof}]
        ...
        \begin{gamefigure}{Adversary $\adversary{C}$ against the IND-CCA security of \kem. Note that both place at most $q_d$ queries to $\dec_O$, one for each decapsulation query placed by $\adversary{A}$.}{fig:adversary-C}
            \begin{games}{2}
                \begin{gameenv}{\textbf{Adversary} $\adversary{C}^{\dec_O(\cdot)}(\pk_1, k^*_1, c^*_1)$}
                    ...
                    \State $k^* \assign H(\hashinputchallenges)$ \label{line:b-real-hqb}
                    \BeginBox[draw=\gfour]
                    \State $k^* \sample \bits^{n}$ \label{line:c-sampled-hqb}
                    % \Comment{$\adversary{C}$}
                    \EndBox
                    \State $c^* \assign (c_1^*, c_2^*)$
                    \State $b' \sample \adversary{A}^{\dec(\cdot)}(\pk, c^*, k^*)$
        ...
        The event of $c_2^*$ not decapsulating to $k_1^*$ in Game 4 can be bound by the correctness of \kem, and the claim follows.
    \item[\emph{Claim 5}]
