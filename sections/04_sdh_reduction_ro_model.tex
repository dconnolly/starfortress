...
\begin{theorem} \label{theorem:sdh_kem-coll_imply_hqb}
    ...
    %
    Then there exist adversaries $\adversary{B}$ and $\adversary{C}$ such
    that,
    %
...
\begin{proof}
    ...
    \item[\textbf{Game 3}]
        ...
        random key.
        %
        With this change, and undoing the changes in $G_1$ and $G_2$, we
        reached the $\cca^1$ game instantiated with \hqb. %
        Therefore,
        %
    \item[\emph{Claim 3}]
        %
        ...
        %
        Undoing the hops accounts for the extra factors in the advantage.
...
\item
    %
    Now that $k^*$ is chosen uniformly at random, which means that $G_3$
